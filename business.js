...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'resolveSession') {
      if (!body.idToken) return res.status(400).json({ error: 'idToken required' });
      let m;
      try {
        m = await authz.resolveMembership(db, body.idToken);
      } catch (e) {
        console.error('[resolveSession]', e && e.message);
        const msg = (e && e.message) || 'Session resolution failed';
        if (/id token|auth/i.test(msg)) return res.status(401).json({ error: 'Invalid or expired sign-in. Please sign in again.' });
        return res.status(500).json({ error: 'Session resolution failed. Check server Firebase Admin configuration.' });
      }
      console.info('[resolveSession]', {
        uid: m.uid,
        isOwner: !!m.isOwner,
        isStaff: !!m.isStaff,
        denied: !!m.denied,
        needsOnboarding: !!m.needsOnboarding,
